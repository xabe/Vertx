...
public class CustomerRepositoryVertxEBProxy implements CustomerRepository {
    ...
    _vertx.eventBus().<JsonArray>request(_address, _json, _deliveryOptions, res -> {
      if (res.failed()) {
        ...
        resultHandler.handle(Future.succeededFuture(res.result().body().stream()
          .map(v -> v != null ? new com.xabe.vertx.mongo.repository.data.Customer((JsonObject)v) : null)
  ...
  public CustomerRepository findById(String id, Handler<AsyncResult<Customer>> resultHandler){
    ...
    _vertx.eventBus().<JsonObject>request(_address, _json, _deliveryOptions, res -> {
      if (res.failed()) {
        ...
        resultHandler.handle(Future.succeededFuture(res.result().body() != null ? new com.xabe.vertx.mongo.repository.data.Customer((JsonObject)res.result().body()) : null));
      }
    });
    return this;
  }
  @Override
  public CustomerRepository save(Customer customer, Handler<AsyncResult<String>> resultHandler){
    if (closed) {
      resultHandler.handle(Future.failedFuture(new IllegalStateException("Proxy is closed")));
      return this;
    }
  ...
  @Override
  public CustomerRepository update(Customer customer, Handler<AsyncResult<Long>> resultHandler){
    if (closed) {
      resultHandler.handle(Future.failedFuture(new IllegalStateException("Proxy is closed")));
      return this;
    }
  ...
  @Override
  public CustomerRepository deleteAll(Handler<AsyncResult<Long>> resultHandler){
    if (closed) {
      resultHandler.handle(Future.failedFuture(new IllegalStateException("Proxy is closed")));
      return this;
    }
  ...
  @Override
  public CustomerRepository deleteById(String id, Handler<AsyncResult<Long>> resultHandler){
    if (closed) {
      resultHandler.handle(Future.failedFuture(new IllegalStateException("Proxy is closed")));
      return this;
    }
